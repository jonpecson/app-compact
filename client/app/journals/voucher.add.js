...
(function() {
    ...
    .controller("voucherAddCtrl", VoucherAddCtrl);
    VoucherAddCtrl.$inject = ['$state', '$scope', '$filter', '$http', 'Contacts', 'Accounts', 'Journals', 'Tax', 'JournalEntries', 'SweetAlert', 'InWords'];

    function VoucherAddCtrl($state, $scope, $filter, $http, Contacts, Accounts, Journals, Tax, JournalEntries, SweetAlert, InWords) {
        var vm = this;
        vm.contacts = [];
        vm.accounts = [];
        ...
        $scope.update = function(tableData, line) {
            ...
            if (line.selectedAccount.acct_id >= 1 && line.selectedContact.contact_id >= 1) {
                ...
                if ($scope.debitTotal || $scope.creditTotal) {
                    ...
                    if ($scope.creditTotal)
                        total = $scope.creditTotal;
                }
